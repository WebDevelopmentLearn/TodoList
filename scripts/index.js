...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
   * Данная функция обновляет чекбексы тасков и добавляет им событие "change".
   * @param checkBox - чекбокс, для которого нужно обновить значение
   * @param obj - объект, в котором находится чекбокс
   * @param datePar - параграфт даты, для которого будут применяться стили
   * @param descPar - параграфт описания, для которого будут применяться стили
   */
  function updateAndHandleCheckbox(checkBox, obj, datePar, descPar) {
  ...
  function editTask(desc, date, obj) {
    ...
    let newEditTaskBtn = editTaskBtn.cloneNode(true);
    ...
    newParagraphElement.addEventListener("keydown", (event) => {
      if (event.key === "Enter") {
        ...
        if (newRegDate.test(event.target.value)) {
          ...
          oldPar.textContent = replaceMonth(event.target.value);
          localStorage.setItem("allTasks", JSON.stringify(allTasks));
          ev.target.parentNode.children[1].children[1].focus();
        } else {
          alert("Неверный формат даты");
          // console.log(replaceMonth(content));
          oldPar.textContent = replaceMonth(content);
        }
      }
    });
  }
  ...
  function editDesc(ev) {
    ...
    newParagraphElement.addEventListener("keydown", (event) => {
      if (event.key === "Enter") {
        ...
        checkPars(
          ev.target.parentNode.children[1].children[0],
          ev.target.parentNode.children[1].children[1],
          isChecked
        );
        localStorage.setItem("allTasks", JSON.stringify(allTasks));
      }
   ...
   *  Если не передать аргумент - будет использоваться тип 'all'
   */
